import { ApiError, OpenMergeError, RequestTimeoutError } from "@openmerge/core"
...
export function failure(error: unknown) {
  ...
  if (error instanceof ApiError) {
    return NextResponse.json(
      { error: error.message, code: error.code, requestId: error.requestId, retryable: error.retryable },
      { status: error.status },
    )
  }
  if (error instanceof RequestTimeoutError) {
    return NextResponse.json({ error: error.message, code: error.code, retryable: true }, { status: 504 })
  }
  ...
  const message = error instanceof Error ? error.message : "Unexpected OpenMerge error"
...
export function stringField(body: Record<string, unknown>, key: string, options: { min?: number; max?: number } = {}) {
  const value = body[key]
  if (typeof value !== "string" || !value.trim()) throw new TypeError(`${key} is required`)
  const normalized = value.trim()
  if (options.min && normalized.length < options.min) throw new TypeError(`${key} must contain at least ${options.min} characters`)
  if (options.max && normalized.length > options.max) throw new TypeError(`${key} must contain at most ${options.max} characters`)
  return normalized
}
